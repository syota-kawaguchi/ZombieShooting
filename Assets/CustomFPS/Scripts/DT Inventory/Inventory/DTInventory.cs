...
namespace DTInventory
{
    ...
    public class DTInventory : MonoBehaviour
    {
        ...
        /// <summary>
        /// Referenced class to provide interaction between input and items
        /// </summary>
        ...
        private void Update()
        {
            //DT FPS Ammo get hook
            if (inventoryItems != null)
            {
                DTFPSInventoryExtended.ammoItems = new List<InventoryItem>();

                foreach (var UIItem in inventoryItems)
                {
                    if (UIItem != null && UIItem.item.type == "Ammo")
                        DTFPSInventoryExtended.ammoItems.Add(UIItem);
                }
            }

            //We bring items left in lootbox back to item list, and remove reference
        ...
        public bool AddItem(Item item)
        {
            ...
            if (_slot)
            {
                ...
                if (autoEquipItems)
                {
                    foreach (var equipmentPanel in equipmentPanels)
                    {
                        if (equipmentPanel.allowedItemType == _InventoryItem.item.type && equipmentPanel.equipedItem == null)
                        {
                            ...
                            _InventoryItem.myEquipmentPanel = equipmentPanel;
                ...
                print("Not enough space to pickup item");
                return false;
            }
        }
        ...
        public void DropItem(InventoryItem InventoryItem)
        {
            ...
            {
                InventoryItem.item.transform.parent = null;
            }
        ...
        public GridSlot CheckFreeSpaceForAllSlots(int width, int height)
        {
            foreach (var slot in slots)
            {
                if (CheckFreeSpaceAtSlot(slot.x, slot.y, width, height) && slot.isLoot == false && slot.equipmentPanel == null)
                {
        ...
        public List<InventoryItem> FindItemsLeftInLoot()
        {
            ...
            if (inventoryItems != null)
            {
                foreach (var item in inventoryItems)
                {
                    if (item != null && FindSlotByIndex(item.x, item.y)!=null && FindSlotByIndex(item.x, item.y).isLoot && !items.Contains(item))
                    {
                        items.Add(item);
                    }
                }
        ...
        public void SearchLootBox(LootBox lootBox)
        {
            ...
            InventoryManager.Instance.mode = InventoryManager.ActiveMode.loot;
            ...
            if (activeLootBox.lootBoxItems != null)
            {
        ...
        {
            print("Use item " + InventoryItem.item.title);
